...
def save_roc_curves(correct_labels, preds, model_name, accuracy, loss, f1):
    ...
    plt.plot(
        fpr,
        tpr,
        label="Model_" + model_name + ' (area = {:.3f})'.format(model_auc)
    )
    ...
    plt.savefig(
        os.path.join(
            OUTPUT_DIR,
            model_name + "_" + opt.dataset +
            "_acc" + str(accuracy * 100) +
            "_loss" + str(loss) +
            "_f1" + str(f1) + ".jpg"
        )
    )
...
def read_frames(video_path, videos):
    ...
    frames_per_video = 30
    frames_interval = int(frames_number / frames_per_video) if frames_number > 0 else 1
...
if __name__ == "__main__":
    ...
    for folder in folders:
        ...
        if not os.path.exists(method_folder):
            print(f"Folder not found: {method_folder}")
            continue
    ...
    with Pool(processes=opt.workers) as p:
        with tqdm(total=len(paths)) as pbar:
            for _ in p.imap_unordered(
                partial(read_frames, videos=videos),
                paths
            ):
                pbar.update()
    ...
    for index, video in enumerate(videos):
        ...
        f.write(
            " --> " + str(video_pred) +
            "(CORRECT: " + str(correct_test_labels[index]) + ")\n"
        )
    ...
    tensor_labels = torch.tensor(
        [[float(label)] for label in correct_test_labels]
    )
